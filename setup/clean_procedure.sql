...
BEGIN
  ...
  FOR INFROMATION_RECORD IN INFROMATION_CURSOR DO
    OBJECT_TO_DELETE := UPPER(INFROMATION_RECORD.USER_NAME)  ;
    DROP USER IF EXISTS identifier( :OBJECT_TO_DELETE ) ;
    OBJECT_TO_DELETE := $$SANDBOX_ROLE_$$||UPPER(INFROMATION_RECORD.USER_NAME)  ;
    DROP ROLE IF EXISTS identifier(OBJECT_TO_DELETE)   ;
    OBJECT_TO_DELETE := $$SANDBOX_DB_$$||UPPER(INFROMATION_RECORD.USER_NAME)  ;
    DROP DATABASE IF EXISTS identifier(OBJECT_TO_DELETE)  ;
    OBJECT_TO_DELETE := $$SANDBOX_WH_$$||UPPER(INFROMATION_RECORD.USER_NAME) ;
